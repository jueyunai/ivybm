...
- 审核只接受 base-owned `pull_request_target` workflow 为当前 head 生成的成功 `CI policy`；候选分支 `pull_request` 运行只是诊断，Draft Fast CI、旧 head、skipped、cancelled、pending 或 failure 都不能授权合并。该 target workflow 仅使用只读权限、不暴露 secrets 或写 token。workflow、`scripts/ci/**`、policy 和镜像触发边界必须由另一名开发者独立 review。
- production image 成功不等于 production 部署授权，部署仍需 jueyunai 人工审批。
- 执行任务时以 `docs/plans/2026-07-16-一期开发实施计划.md` 为总计划；管理后台 Portal 事项以 `docs/architecture/adr/0004-modular-admin-portal.md` 和 `docs/plans/2026-07-29-modular-admin-portal-implementation.md` 为准。jueyunai 负责 Portal Core、官网/CMS/素材/线索和 AI 内容工作台（页面、生成/审核流程及 Generated/Review/Publish 共享结构）；xuemusi 负责知识/AI 调试、AI 客服/统一会话、平台账号/连接器/readiness 及真实发布服务。完成阶段后更新 `docs/开发进度.md`。
