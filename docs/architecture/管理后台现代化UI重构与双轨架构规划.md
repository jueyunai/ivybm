...
IVYBM 管理后台采用“一个后端控制平面、两个前端体验层、一个模块化运营门户”的架构：

- Payload CMS 继续负责数据模型、Auth、RBAC、Collections、migration、审计、媒体和 Local API；
- `/dashboard` 是根据 Digital Lattice 设计稿自研的日常运营门户；
- `/admin` 保留为 Payload 技术后台、复杂 CMS 表单和故障回退入口；
- 官网、运营门户和技术后台仍运行在同一个 Next.js + Payload 模块化单体中；
- 门户基座先交付，业务模块按价值、依赖和负责人逐个挂载，不追求一次完成全部页面。

这里的“双轨”不是维护两套业务系统。数据库、账号、权限、领域状态机、Jobs 和外部平台接口只有一套；
双轨仅指“运营人员的自研体验”和“技术人员的 Payload 控制面”并存。
...
- 已有 `/admin` Nav、Operations Dashboard、账户菜单和回归测试；
...
| 技术后台 | `/admin` | Payload CRUD、复杂字段、用户、凭据、模型路由、底层记录 | 作为最终运营体验目标 |
...
- Registry：模块 ID、路由、菜单、负责人、成熟度、fallback、feature flag；
...
│       ├── platforms/
│       └── operations/
├── admin/                 # 既有 Payload /admin 扩展，继续保留
...
- 现有 `/admin` E2E 必须证明未被 Portal CSS 污染。
...
红色边界对应用户原图的红圈范围；箭头表示数据或业务依赖，不改变模块所有权。
...
flowchart TB
    ...
    class CORE,CMS,ASSET,LEAD,SYS,DESIGN,SLOT owner
    class KNOW,STUDIO,PUB,CHAT,INBOX,SOCIAL collaborator
...
- jueyunai 先交付 Core、示例模块、开发指南和 UI contract；
- xuemusi 在基座稳定后独立负责红圈模块的页面、读模型、命令和后续迭代；
- jueyunai 对协作者模块做 IA、视觉一致性和集成验收，不接管其领域逻辑；
...
| P1-1 | AI 内容生产、审核与发布准备 | 建立可追溯的内容流 | 素材/知识 + 正式 Generated/Review/Publish 结构 | xuemusi |
...
先交付官网内容 Hub 和素材库。复杂富文本、多语言和凭据配置暂时深链 `/admin`，
Portal 先解决发现、筛选、预览、完整度和任务上下文。

### 阶段 C：协作者模块接入

由 jueyunai 先交付模块开发指南、示例 manifest、权限/状态测试工具和 UI primitive。xuemusi 先接知识库与 AI 调试，
再接统一会话；这两块后端最成熟，能尽早验证基座是否真的可插拔。
...
1. 用户只登录一次即可在 `/dashboard` 与有权限的 `/admin` 页面间切换；
2. 关闭 Portal 总开关后可以无数据迁移地回退到 `/admin`；
...
8. Digital Lattice 设计在桌面和窄屏无溢出，`/admin` 视觉未被 Tailwind 污染；
9. production 部署拓扑、数据库、媒体和 worker 不因 Portal Core 增加新服务；
10. 文档中不再同时存在“`/admin` 唯一运营入口”和“`/dashboard` 当前目标”两套 active 基线。

## 15. 关联文档

- [ADR-0004：Payload 控制平面与模块化运营门户双轨架构](adr/0004-modular-admin-portal.md)
