...
一期图文发布范围仅为 Facebook、Instagram、LinkedIn。Facebook / Instagram 使用企业账号和 Meta 发布权限；LinkedIn 账号类型不限制，但自动发布仍以获得 API 发布权限为前提。TikTok 发布、视频自动发布和 WhatsApp 发布不在一期范围。

2C4G 服务器不适合常驻浏览器自动化。发布策略建议分三档：

| 档位            | 方式                                  | 一期建议                           |
| --------------- | ------------------------------------- | ---------------------------------- |
| 官方 API 发布   | Facebook / Instagram / LinkedIn 等获得权限的渠道走官方接口 | 优先                               |
...
- PostgreSQL 一期放本机 Docker volume；当前开发阶段显式延期备份，禁止不可逆 migration 或写入不可替代客户数据；
...
3. 试运营：观察磁盘、内存和 uploads 增长；自动备份启用前不承诺 RPO / RTO。
4. 后续扩容：系统盘容量紧张时，优先迁移 uploads/PDF/图片到 COS；在接收不可替代数据前启用外部备份。
...
| 平台 API 审核不可控 | Meta / TikTok / LinkedIn 权限或审核卡住 | 官网聊天和飞书先闭环；一期指定平台按 PoC / 半自动路径验收                  |
...
- 在首次接收不可替代客户数据前，必须重新启用每日 `pg_dump`、uploads 增量、外部存储和恢复演练，并另行确认留存周期与 RPO / RTO。
