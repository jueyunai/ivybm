...
| 控制平面 / CMS   | Payload CMS                            | 唯一数据模型、Auth、RBAC、文件上传、草稿、版本、多语言、Local API 和审计；已有 `/admin` 仅供内部维护                           |
...
| Payload 原生 Admin + 项目 Custom Views | 已有内部维护能力，停止一期扩展 | 已落地代码仅作内部维护与安全回归；不进入 Portal 导航、设计、开发或验收。 |
...
后续：第一阶段只建设 `/dashboard` 体验层；已有 `/admin` 仅供内部维护，不新增 UI，也不作为 Portal fallback。
...
Payload 控制平面、权限、领域服务和数据，内部维护入口不属于产品验收。**
...
- 一期运营门户使用同一应用的 `/dashboard` 并由 Payload 鉴权保护；已有 `/admin` 仅供内部维护且不对普通运营用户暴露。
