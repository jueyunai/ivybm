...
本文是一期 CMS 与运营后台的选型主文档。后台 UI 的详细决策记录见 [`ADR-0002`](adr/0002-admin-ui-composition.md)，第三方主题的试装证据见 [`payload-theme POC 记录`](../research/payload-theme-poc-record.md)；三者共同构成唯一的选型依据链路。
...
| 方案 | 本轮结论 | 适用性与取舍 |
| --- | --- | --- |
| **Payload 原生 Admin + 项目 Custom Views** | **一期采用** | 原生页面继续处理 CMS CRUD、复杂字段和系统配置；自有 Dashboard / Workspace 只处理高频任务，复用现有登录、权限、审计和领域服务。新增页面必须使用 Payload 公开扩展点、服务端角色检查和有界 Local API。 |
...
| Refine / React-admin / Ant Design Pro 独立后台 | 一期不采用，满足条件后再评估 | 它们适合已有稳定业务 API、需要独立发布节奏或复杂数据应用的场景；当前接入会重复实现 Payload 会话、权限呈现、草稿/版本、关系字段、错误处理和命令边界，增加第二个前端运行面。 |
| 完全重写自有后台 | 不采用 | 可获得最大布局自由度，但会把成熟 CMS 基础设施变为自维护范围；对小团队和 2C4G 单机的风险、工期和长期升级成本均不成比例。 |

第一性原理结论：一期应该把有限 UI 研发投入到「十秒内发现正确任务、在上下文内安全完成命令」这两个差异化问题，而不是重复构建身份、数据录入和 CMS 管理能力。只有当独立工作台已承担绝大多数日常操作、Payload 壳成为验证过的交互瓶颈，且稳定的领域 API / 鉴权契约已就绪时，才评估迁移到独立运营端。
...
| 接口                | 消费方                                    | 提供方                                 | 边界                                                                                                         |
| ------------------- | ----------------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `ChatService`       | jueyunai 的官网 ChatWidget 和运营接管界面 | xuemusi 的会话 / AI / 人工接管领域服务 | 前端提交消息与接管命令，消费权威 handoff 状态和稳定错误码；不直接访问模型 SDK、会话表、负责人或审计字段      |
...
后台 UI 的进一步组合决策：一期保留 Payload 原生 CMS CRUD，运营 Dashboard、会话接管、内容审核和发布采用项目自有 Custom Views；不安装或 fork `payload-theme`，不另建 Refine / React-admin 独立后台。详细的约束、后果和重新评估条件见 [`ADR-0002：Payload 原生 Admin 与自有 Custom Views`](adr/0002-admin-ui-composition.md)。
...
UI: Project CSS tokens + Payload Admin custom components / Custom Views
