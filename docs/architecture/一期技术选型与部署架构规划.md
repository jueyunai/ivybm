...
| 控制平面 / CMS   | Payload CMS                            | 唯一数据模型、Auth、RBAC、文件上传、草稿、版本、多语言、Local API、审计和技术后台 `/admin`                                   |
...
- Payload 继续提供统一后端和技术后台；`/dashboard` 在同一应用内提供模块化运营体验，不复制认证或数据模型；
...
| Payload 原生 Admin + 项目 Custom Views | 已完成首轮，保留为技术入口/fallback | 已落地 Nav、Operations Dashboard 与账户菜单；继续服务 `/admin`，但不再作为最终运营 UI 的唯一壳。 |
...
flowchart TB
    ...
    TECH["Payload 技术后台 /admin<br/>复杂 CRUD / 用户 / 凭据 / 调试"] --> APP
    CORE --> APP
    CORE --> CMS
    CORE --> KB
    CORE --> MSG
...
按静态 module registry 挂载；复杂表单和技术配置回退到 `/admin`。
...
| `PublishingService` | xuemusi 的 AI 内容生产/审核/发布模块                  | xuemusi 的第三方平台发布服务           | 工作台查询平台能力、提交已审核发布任务、读取发布状态；平台 SDK、token、限流和结果回调封装在服务端 adapter 内 |

两个接口都必须先提供 TypeScript port/interface、请求响应 schema、错误码、状态枚举、fake service 和 contract fixture。jueyunai 可以在真实后端完成前使用 `ChatService` mock 完成官网 ChatWidget；xuemusi 使用 fake service/repository 并行完成统一会话、内容工作台、平台验签、幂等与 adapter。jueyunai 对协作者 Portal 模块负责 IA、视觉和集成验收，不接管其领域逻辑。破坏性 contract 变更必须由双方 review 并同步更新 mock、contract test 和消费者。
...
Task 12 由 xuemusi 定义内容生成、审核状态机以及 `GeneratedContents` / `ContentReviews` / `PublishJobs` / `PublishLogs` 共享结构，并实现页面、领域服务和平台 adapter。真实对外发布命令必须等正式结构、账号 readiness 和补偿 contract 都就绪；在此之前页面只能使用 mock 展示或标记 `dependency-gated`。
...
| 6. AI 内容工作台          | 18-20 | xuemusi 实现文案/脚本生成、人工审核、发布准备页面和内部任务；jueyunai 负责 IA/视觉/集成验收                         |
...
UI: /dashboard Portal Core + scoped Tailwind/shadcn source components; /admin keeps Payload UI
...
验收口径：**客户只登录一次，即可在 `/dashboard` 完成日常运营，并在有权限时进入
`/admin` 处理复杂技术配置；两种体验共享同一 Payload 控制平面、权限、领域服务和数据。**
