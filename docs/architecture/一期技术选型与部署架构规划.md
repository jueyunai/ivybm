...
| 部署方式         | Docker Compose + 1Panel 手动拉镜像     | GitHub CI 推送 SHA 镜像到私有仓库（默认 GHCR），1Panel 手动 pull / redeploy；避免 Kubernetes、微服务网关、重型监控 |
...
- 当前不设置常驻 staging；本地测试与 GitHub CI 是上线前质量门禁，production 由 1Panel 手动拉取已验证的 immutable SHA 镜像发布。
...
- 镜像在 GitHub CI 构建，使用 Git commit SHA 标记并推送私有镜像仓库（默认 GHCR）；服务器只通过 1Panel 手动拉镜像，不安装开发依赖、不现场构建。
- 不设置 staging：本地测试和 CI 质量门禁通过后，由负责人手动选择 immutable SHA 在 production 执行 migration、健康检查和 smoke test。
