...
- Facebook Page OAuth 使用 Facebook Login for Business 的 `config_id` 发起授权，App Secret 仅出现在服务器到 Meta 的 POST body；回调 URL 运行时绑定登记的站点 origin，不信任请求 Host。state / account context 以平台凭据主密钥加密，Page identity 与 `pages_show_list`、`pages_manage_metadata`、`pages_messaging`、`pages_read_engagement` 在 Token 入库前逐项验证。Facebook 发布权限不作为 Messenger 连接硬门槛。Instagram 从该流程拆开，改走独立 Instagram Login for Business；初始 code exchange 解析 provider 返回的实际 `permissions`，不调用未受支持的 `graph.instagram.com/.../me/permissions`，再用 long token exchange 与 `/me` 完成 token 生命周期和身份绑定。
