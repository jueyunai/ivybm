...
| MIG-001 | Migration | 一次性隔离 PostgreSQL 18 + pgvector 验证库 | full up/down/up | 仅验证 migration 线性可执行；`down` 的破坏性仅限 disposable 库。Production rollback 只回滚应用并保留 schema/data | migration log |
...
- 回归：同步最新 `origin/main` 后 unit 115 files / 806 tests、integration 29 files / 172 tests、contract 7 files / 70 tests、知识 Portal Chromium E2E 3/3（无 skip）、build、migration down/up 均通过；非 Docker operations 32/32 通过。
...
- 外部缺口：本机未安装 Docker，Compose 8 项和 production preflight 成功路径 9 项无法完成最终 Compose 校验，交由 GitHub Full CI；未获 production 部署批准。
