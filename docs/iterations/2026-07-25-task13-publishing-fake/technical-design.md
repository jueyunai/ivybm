...
- `src/modules/platforms/linkedin/package.ts` 以 store-only ZIP 生成文案、无敏感 URL 的 manifest 和调用方提供的素材字节；固定 DOS 时间、稳定排序和 CRC32 使相同输入得到相同下载文件。它不读取磁盘、下载 `sourceUrl` 或调用 LinkedIn。

## 平台默认能力

- Facebook / Instagram：`conditional + automatic` 仅表示能力路径；默认无账号 fake 返回 `account_not_connected`。测试 / UI 只有显式传入 `available + automatic` override 后才可模拟成功，绝不表示真实平台可用。
