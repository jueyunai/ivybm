...
const aiProviderPort = Number(process.env.IVYBM_E2E_AI_PROVIDER_PORT)
if (
  !isExternalReadOnly &&
  (!Number.isInteger(aiProviderPort) || aiProviderPort < 1 || aiProviderPort > 65_535)
) {
  throw new Error('Mutation E2E requires a launcher-owned AI provider port')
}
...
export default defineConfig({
  ...
  webServer: isExternalReadOnly
    ...
    : [
        {
          command: 'node scripts/e2e/start-ai-provider-stub.mjs',
          reuseExistingServer: false,
          timeout: 30_000,
          url: `http://127.0.0.1:${aiProviderPort}/health`,
        },
        {
