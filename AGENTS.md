...
- 若当前 checkout 存在 `.codex/ivybm-local-agent-rules.md`，本地代理应在阅读本文件后继续读取它；该文件是操作者本机的会话调度和编码偏好，不属于仓库交付物。
- 该文件可能不存在于其他开发者或远程协作者的 clone 中；不存在时直接跳过，不构成缺失依赖或阻塞。远程协作者继续遵守本文件、`CONTRIBUTING.md`、实施计划和其团队工具规范。
- 本地覆盖层只能补充任务编排、worktree 分配、会话汇报和本地验证偏好，不得放宽或覆盖本文件中的安全边界、模块 ownership、共享结构 Review、PR/CI、migration、production 审批和敏感资料规则。
