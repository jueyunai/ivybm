...
PR 用于审查已经稳定的结果，不用于替代本地调试。每个 checkpoint 按以下顺序执行：

1. 在独立 worktree 编写 E2E 和必要的最小生产修复，先跑真实本地主流程。
2. 对每次失败分类：测试定位或断言问题、环境 blocker、生产代码缺陷；生产缺陷必须与回归测试一起修复。
3. 在创建 Draft PR 前冻结目标、Task、owner、reviewer、共享契约、依赖、回滚和明确不包含范围；范围未冻结时只保留本地提交。
4. 按业务目标和可独立回滚单元创建 PR，不按测试文件、commit 或 reviewer 机械拆分；依赖 PR 必须标明 base 和合并顺序。
5. push 前集中完成同一轮小修改并运行定向门禁；Review 后集中处理可归并意见，再对最新 head 一次性更新，禁止逐条意见逐次触发 CI 调试。
6. 缺少真实账号、数据库或受控环境时，在 PR 中记录 blocker 和已有证据，不把 skipped、fake 或未运行结果写成真实联调通过。

本地可以使用临时整合 worktree 整理最终提交，但不保留长期 integration / develop 分支，也不把未冻结的探索分支推到远程。
