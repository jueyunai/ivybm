...
- 2026-08-09 的 CI 可信锚 bootstrap PR 是上述规则的唯一一次迁移例外，具体范围见 `docs/plans/2026-08-09-ci-trusted-bootstrap-plan.md`。该 PR 新增的 `pull_request_target` 在进入 `main` 前不会运行，必须以固定 base / 最终 head、本地完整门禁、权限扫描、完整 diff、revert 最终 PR merge commit 的回滚证据和双方明确确认替代自动 trusted check；head 变化即重新确认，合并或关闭后例外立即失效。PR #64 及后续 PR 必须使用合入后的真实 `CI policy`，不得继承该例外。
