...
export class FacebookE2EHarness {
  ...
  async runPublicationUntilIdle(): Promise<void> {
    ...
    await this.runNext()
    const finalOutcome = await this.runNext()
    if (finalOutcome !== 'idle')
      throw new Error('Facebook publication E2E worker did not become idle')
  }
  ...
  async cleanup(): Promise<CleanupSentinels> {
    ...
    try {
      ...
      try {
        ...
        const publicationQueueJobs =
          publicationJobIDs.length > 0
            ...
            : { rows: [] }
        const publicationQueueJobIDs = publicationQueueJobs.rows.map(({ id }) => id)
        const auditTargets: Array<[resource: string, ids: number[]]> = [
          ...
          ['content-reviews', this.contentIDs],
          ['publish-jobs', publicationJobIDs],
          ['publish-logs', publicationJobIDs],
        ]
        ...
        if (publicationQueueJobIDs.length > 0) {
          await client.query('DELETE FROM jobs WHERE id = ANY($1::int[])', [publicationQueueJobIDs])
        }
      ...
      for (const id of this.knowledgeDocumentIDs) {
        await this.payload
          .delete({ collection: 'knowledge-documents', id, overrideAccess: true })
          .catch(() => undefined)
      }
