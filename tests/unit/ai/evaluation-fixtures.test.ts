...
describe('AI evaluation fixtures', () => {
  ...
  it('passes the structural and P0 safety gate', () => {
    ...
    expect(fixtures.cases.filter((item) => item.risk_level === 'P0')).toHaveLength(7)
  ...
  it('reports the seed-set coverage gap without treating it as a validation failure', () => {
    const report = createCoverageReport(fixtures.cases, fixtures.sources)

    expect(report).toContain('样例总数：10')
    expect(report).toContain('P0 样例：7')
    expect(report).toContain('低于一期最小建议 60 条')
