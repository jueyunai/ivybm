...
export async function generateMetadata({
  ...
  return buildPageMetadata({
    description: page?.summary || copy.pages.contactSubtitle,
    locale: value,
    media: page?.heroImage || home?.heroImage,
    path: '/contact',
    seo: page?.seo || settings.defaultSeo,
    siteName: settings.siteName,
    title: page?.title || copy.navigation.contact,
  })
}
...
export default async function ContactPage({
  ...
  const initialInterest = copy.contact.productOptions.some(
    ([optionValue]) => optionValue === product,
  )
    ? product
    : ''
  const cards = [
  ...
  return (
    <>
      <PageHero
        image={page.heroImage || home?.heroImage}
        subtitle={page.summary || copy.contact.subtitle}
        title={page.title}
      />
          ...
          <div className="grid">
            {cards.map(({ Icon, title, value }) => (
              <article className="info-card" key={title}>
                <Icon aria-hidden size={28} stroke={1.7} />
                <h3>{title}</h3>
                <p className="muted pre-line">{value}</p>
              </article>
            ))}
          </div>
          <InquiryForm
