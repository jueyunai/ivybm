...
function LeadEditor({ mode, onClose, onDone, options, role, selected, text }: { mode: EditorMode; onClose: () => void; onDone: (message: string, mutation: LeadMutation) => void; options: LeadsSummary['options']; role: PortalRole; selected: LeadSummaryItem | null; text: typeof copy['zh'] | typeof copy['en'] }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const response = await fetch(mode === 'create' ? '/api/portal/leads' : `/api/portal/leads/${form.id}`, { body: JSON.stringify(mutation), credentials: 'same-origin', headers: { 'content-type': 'application/json' }, method: mode === 'create' ? 'POST' : 'PATCH' })
  ...
  const remove = async () => {
    if (!form.id) return
    setBusy(true); setError(null)
    try { const response = await fetch(`/api/portal/leads/${form.id}`, { body: JSON.stringify({ updatedAt: form.updatedAt }), credentials: 'same-origin', headers: { 'content-type': 'application/json' }, method: 'DELETE' }); const body = await response.json() as { error?: { message?: string } }; if (!response.ok) throw new Error(body.error?.message || text.formError); onDone(text.deleted, { deleted: true, id: form.id }) } catch (caught) { setError(caught instanceof Error ? caught.message : text.formError) } finally { setBusy(false) }
  }
