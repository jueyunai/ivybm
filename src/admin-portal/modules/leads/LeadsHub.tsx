...
const relatedConversationStatus = (status: string, text: typeof copy['zh'] | typeof copy['en']): string =>
  ({
    ai_active: text.state.new === '新增' ? 'AI 服务中' : 'AI active',
    handoff_requested: text.state.new === '新增' ? '待接管' : 'Awaiting takeover',
    human_active: text.state.new === '新增' ? '人工处理中' : 'Human active',
    resolved: text.state.new === '新增' ? '已解决' : 'Resolved',
  } as Record<string, string>)[status] ?? (text.state.new === '新增' ? '状态未知' : 'Unknown status')
  ...
  const save = async () => {
    ...
    try {
      ...
      onDone(text.saved, {
        ...
        ...(mode === 'edit' ? { values: { assignedTo: form.assignedToId ? Number(form.assignedToId) : null, company: form.company || null, country: form.country || null, email: form.email, interest: form.interest || null, intentLevel: form.intentLevel as LeadSummaryItem['intentLevel'], message: form.message, name: form.name, phone: form.phone || null, source: Number(form.sourceId), status: form.status as LeadSummaryItem['status'] } } : {}),
      })
