...
export function AiSettingsPanel({ initialSummary }: { initialSummary: PortalAiSettingsSummary }) {
  ...
  return (
    ...
    >
          ...
          return (
            <article key={item.key}>
              <div>
                ...
                <small>
                  {item.reason
                    ? messages.readinessReason[item.reason]
                    : pending
                      ? messages.configuredPendingVerification
                      : messages.enabled}
                </small>
              </div>
              <StatusBadge
      ...
      {feedback ? (
        <p
          className={`portal-ai-settings__feedback is-${feedback.tone}`}
          ...
          providers.map((provider) => (
            <article key={provider.id}>
              <div>
                ...
                <small>
                  {provider.apiKeyConfigured ? messages.apiKeyConfigured : messages.actionRequired}
                </small>
              </div>
              <StatusBadge
                label={provider.enabled ? messages.enabled : messages.disabled}
...
function ProviderForm({
  ...
  onCancel: () => void
  onSave: SaveHandler
  provider: PortalAiProviderSummary | null
}) {
  ...
  return (
    <form className="portal-ai-settings__form" onSubmit={submit}>
      <header>
        <h4>{provider ? messages.edit : messages.newProvider}</h4>
      </header>
      <label>
        <span>{messages.providerName}</span>
        <input
          disabled={busy}
          maxLength={100}
      ...
      <label>
        ...
        <input
          ...
          value={apiKey}
        />
        <small>{messages.apiKeyDescription}</small>
      </label>
  ...
  return (
    <div className="portal-ai-settings__workspace">
      <div className="portal-ai-settings__list">
        <div className="portal-ai-settings__list-heading">
          ...
          <Button
            ...
            variant="secondary"
          >
            <IconPlus size={15} stroke={1.8} /> {messages.newModel}
          </Button>
        </div>
        {profiles.length ? (
          profiles.map((profile) => (
            <article key={profile.id}>
              <div>
                ...
                <small>
                  {profile.providerName ?? messages.actionRequired} ·{' '}
                  {messages.capabilities[profile.capability]}
                </small>
              </div>
              <StatusBadge
                label={profile.enabled ? messages.enabled : messages.disabled}
      ...
      {showForm ? (
        <ProfileForm
          busy={busy}
          key={existing ? `profile-${existing.id}` : 'profile-new'}
          messages={messages}
          onCancel={() => {
            setCreating(false)
            onCancel()
...
function ProfileForm({
  ...
  onCancel: () => void
  onSave: SaveHandler
  profile: PortalAiModelProfileSummary | null
  providers: PortalAiProviderSummary[]
}) {
  ...
  const [reasoningEffort, setReasoningEffort] = useState(
    profile?.parameters.reasoningEffort ?? 'medium',
  )
  const [reasoningEnabled, setReasoningEnabled] = useState(
    profile?.parameters.reasoningEnabled ?? false,
  )
    ...
    await onSave(
      {
        ...
        name,
        parameters: {
          dimensions: capability === 'embedding' ? Number(dimensions) : null,
          maxOutputTokens: capability === 'text' ? toOptionalNumber(maxOutputTokens) : null,
          reasoningEffort,
          reasoningEnabled: capability === 'text' && reasoningEnabled,
          temperature: capability === 'text' ? toOptionalNumber(temperature) : null,
          timeoutMs: Number(timeoutMs),
          topP: capability === 'text' ? toOptionalNumber(topP) : null,
        },
        providerID,
  ...
  return (
    <form className="portal-ai-settings__form" onSubmit={submit}>
      <header>
        <h4>{profile ? messages.edit : messages.newModel}</h4>
      </header>
      <div className="portal-ai-settings__form-grid">
        ...
        {capability === 'text' ? (
          <>
            <label>
              <span>{messages.maxOutputTokens}</span>
              <input
                disabled={busy}
                max={128000}
                min={1}
            ...
            <label>
              <span>{messages.temperature}</span>
              <input
                disabled={busy}
                max={2}
                min={0}
      ...
      {capability === 'text' ? (
        <label className="portal-ai-settings__check">
          <input
            ...
            type="checkbox"
          />
          <span>{messages.reasoningEnabled}</span>
        </label>
      ) : null}
      <label className="portal-ai-settings__check">
        <input
      ...
      <div className="portal-ai-settings__list">
        <div className="portal-ai-settings__list-heading">
          ...
          <Button
            ...
            variant="secondary"
          >
            <IconPlus size={15} stroke={1.8} /> {messages.newRoute}
          </Button>
        </div>
        {routes.length ? (
          routes.map((route) => (
            <article key={route.id}>
              <div>
                <strong>{route.usageKey}</strong>
                <span>{route.profileName ?? messages.actionRequired}</span>
                <small>{messages.capabilities[route.operation]}</small>
              </div>
              <StatusBadge
                label={route.enabled ? messages.enabled : messages.disabled}
          ...
          <p className="portal-ai-settings__empty">{messages.noRoutes}</p>
        )}
      </div>
      {showForm ? (
        <RouteForm
          busy={busy}
          key={existing ? `route-${existing.id}` : 'route-new'}
          messages={messages}
          onCancel={() => {
            setCreating(false)
            onCancel()
...
function RouteForm({
  ...
  onSave: SaveHandler
  profiles: PortalAiModelProfileSummary[]
  route: PortalAiUsageRouteSummary | null
}) {
  ...
  const operation: PortalAiCapability =
  ...
  const compatible = useMemo(
    () => profiles.filter((profile) => profile.capability === operation),
    [operation, profiles],
  )
  const [profileID, setProfileID] = useState(route?.profileID ?? compatible[0]?.id ?? 0)
  const normalizedProfileID = compatible.some((profile) => profile.id === profileID)
    ? profileID
    : (compatible[0]?.id ?? 0)
  const submit = async (event: FormEvent) => {
    event.preventDefault()
    await onSave(
      { enabled, operation, profileID: normalizedProfileID, updatedAt: route?.updatedAt, usageKey },
      route?.id ?? null,
    ).catch(() => undefined)
  }
  return (
    <form className="portal-ai-settings__form" onSubmit={submit}>
      <header>
        <h4>{route ? messages.edit : messages.newRoute}</h4>
      </header>
      <label>
...
const FormActions = ({
  ...
  <div className="portal-ai-settings__form-actions">
    <Button disabled={busy} type="submit">
      <IconDeviceFloppy size={16} stroke={1.8} /> {messages.save}
    </Button>
    <Button disabled={busy} onClick={onCancel} variant="ghost">
      <IconX size={16} stroke={1.8} /> {messages.cancel}
    </Button>
  </div>
)
