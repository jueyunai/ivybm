import {
  ...
  type AssistedPublicationPackage,
  type AssistedPublicationPackageAsset,
  type AssistedPublicationRequest,
  type PreparedAssistedPublication,
...
const compareAssetIDs = (
  left: AssistedPublicationAsset,
  right: AssistedPublicationAsset,
...
const safeBaseName = (value: string): string => {
  ...
  if (
    ...
    throw new PublishingContractValidationError(
      'LinkedIn assisted package file names must be safe base names',
    )
  }
  return name
}
...
const packageAsset = (asset: AssistedPublicationPackageAsset): AssistedPublicationPackageAsset => {
  if (!(asset.bytes instanceof Uint8Array)) {
    throw new PublishingContractValidationError(
      'LinkedIn assisted package assets require Uint8Array bytes',
    )
  }
  const fileName = safeBaseName(asset.fileName)
  const { sourceUrl: _sourceUrl, ...normalized } = normalizePublicationAsset({
    ...asset,
    fileName,
  })
  return {
...
const assertInputFitsPackageBudget = (
  ...
  if (inputBytes > MAX_PACKAGE_BYTES) {
    throw new PublishingContractValidationError(
      `LinkedIn assisted package exceeds ${MAX_PACKAGE_BYTES} bytes`,
    )
  }
...
export const createLinkedInAssistedPackage = ({
  ...
  const assetEntries = exportData.assets.map((asset, index) => {
    ...
    if (!bytes) {
      throw new PublishingContractValidationError(
        'LinkedIn assisted package asset bytes are missing',
      )
    }
    return {
...
export const createLinkedInAssistedPreparation = (
  input: AssistedPublicationRequest,
): PreparedAssistedPublication => {
  const request = normalizeAssistedPublicationRequest(input)
  const assets = request.assets.map(({ bytes: _bytes, ...asset }) => asset)
  return {
    artifact: createLinkedInAssistedPackage({ assets: request.assets, text: request.text }),
    manifest: createLinkedInAssistedExport({ assets, text: request.text }),
    mode: 'assisted',
    platform: 'linkedin',
