...
export const resolveKnowledgeTranslationPrompt = async ({
  ...
  const prompts = result.docs
    ...
    .map((value): TranslationPrompt => ({
      ...
      version: typeof value.version === 'number' && Number.isSafeInteger(value.version) ? value.version : 0,
    }))
  ...
  if (candidates.length === 0) {
    throw new KnowledgeTranslationError('translation-prompt-unavailable', 'No active translation prompt is configured')
  }
  ...
  if (latest.length !== 1) {
    throw new KnowledgeTranslationError('translation-prompt-ambiguous', 'The active translation prompt is ambiguous')
  }
  return latest[0]
}
...
export const splitKnowledgeTranslationText = (
  ...
  if (!Number.isInteger(maximum) || maximum < 10) throw new RangeError('translation chunk budget is invalid')
  const normalized = text.replace(/\r\n?/g, '\n').trim()
  if (!normalized) throw new KnowledgeTranslationError('empty-source-text', 'No source text is available for translation')
  const chunks = normalized
    .split(/\n{2,}/)
    .flatMap((paragraph) => splitAtBoundary(paragraph, maximum))
    .filter(Boolean)
  if (chunks.length > KNOWLEDGE_TRANSLATION_MAX_CHUNKS) {
    throw new KnowledgeTranslationError('translation-too-large', 'The source is too large to translate safely')
  }
  return chunks
}

const renderInstructions = (template: string, sourceLocale: string, targetLocale: KnowledgeTranslationLocale): string => {
  ...
  const rendered = template.replace(/\{\{?([a-zA-Z][a-zA-Z0-9_]*)\}?\}/g, (match, key: string) => substitutions[key] ?? match)
...
const requiredFidelityTokens = (value: string): string[] => {
  ...
  const tokens = [...new Set(patterns.flatMap((pattern) => value.match(pattern) ?? []))]
...
export const translateKnowledgeText = async ({
  ...
  for (const chunk of chunks) {
    if (signal?.aborted) throw signal.reason instanceof Error ? signal.reason : new Error('Translation aborted')
    ...
    if (!result.text.trim()) throw new KnowledgeTranslationError('translation-empty', 'The translation provider returned empty text')
    if (shielded.markers.some(([marker]) => !result.text.includes(marker))) {
      throw new KnowledgeTranslationError('translation-fidelity', 'The translation provider changed a required number or image placeholder')
    }
  ...
  if (!model) throw new KnowledgeTranslationError('translation-model-unavailable', 'The translation model is unavailable')
  return { locale: targetLocale, model, promptVersion: prompt.version, text: translated.join('\n\n') }
}
